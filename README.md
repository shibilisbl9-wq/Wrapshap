...
**Realistic mockups.** Design 1 has two photo-style renders, `-photo-back.jpg` and `-photo-front.jpg`: the shirt laid flat on
grey paper under soft studio light. These are 3D renders (Blender Cycles), not photographs: the shirt is modelled with folds,
body volume, a rib collar, stitched hems and seams, and the real print files are mapped onto the fabric, so the artwork is exact
and follows the folds. The same generator (`source/photo_mockup.py`) can render the other five designs and a six-shirt board.
It was stopped before those finished. The on-model lookbook photos were made in Higgsfield (GPT Image 2) and are in the Higgsfield account,
not in this repo. Those are AI images that redraw the print, so check the logo in each against the original before using one.
